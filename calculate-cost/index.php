				...
				<ul class="q-calc__list" :class="selectedCategory != null ? 'active' : ''">
					...
					<li>
						<div @click="calculate" class="l-calc__action q-calc__action " :class="selectedCategory != null ? 'active' : ''">
							<a v-if="!summ.other" class="btn btn-yellow" href="javascript:void(0)">Рассчитать</a>
							<a v-else class="btn btn-yellow l-btn__wa-calc l-wa__row-action svg" :class="summ.other ? 'active' : ''"  href="https://wa.me/79681507002?text=Здравствуйте!%20Пишу%20с%20сайта%20aian14.ru" target="_blank">
								<svg>
									<use xlink:href="<?=SITE_TEMPLATE_PATH?>/dist/img/sprite.svg#sprite--icon-wa" />
								</svg>
								Заказать звонок
							</a>
						</div>
